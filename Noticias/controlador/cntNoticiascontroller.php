...
include('../modelos/modNoticiasModel.php');
...
try {
    switch ($action) {
        case 'list':
            $rows = $model->getAllNoticias();
            header('Content-Type: application/json; charset=utf-8');
            echo json_encode(['success' => true, 'data' => $rows], JSON_UNESCAPED_UNICODE);
            break;

        case 'get':
            $id = intval($_GET['idNoticia'] ?? 0);
            $row = $model->getNoticias($id);
            header('Content-Type: application/json; charset=utf-8');
            if ($row) {
        ...
        case 'insert':
            ...
            $epigrafe = $_POST['txtEpigrafe'] ?? '';
            $contenido = $_POST['atxtDescripcion'] ?? '';
            $imagenNombre = null;
            if (!empty($_FILES['imagen']['tmp_name'])) {
                $imagenNombre = $model->guardarImagen($_FILES['imagen']);
            }
            $ok = $model->insertNoticias($titulo,$autor,$epigrafe, $contenido, $imagenNombre);
            header('Content-Type: application/json; charset=utf-8');
            echo json_encode(['success' => $ok, 'message' => $ok ? 'Insertado' : 'No se pudo insertar'], JSON_UNESCAPED_UNICODE);
            break;

        case 'update':
            $id = intval($_POST['idNoticia'] ?? 0);
            $titulo = $_POST['txtTitulo'] ?? '';
            $autor = $_POST['txtAutor'] ?? '';
            $epigrafe = $_POST['txtEpigrafe'] ?? '';
            $contenido = $_POST['atxtDescripcion'] ?? '';
            ...
            if (!empty($_FILES['imagen']['tmp_name'])) {
                ...
                $imagenNombre = $model->guardarImagen($_FILES['imagen']);
                if ($anterior && !empty($anterior['imgNoticia']) && $anterior['imgNoticia'] !== $imagenNombre) {
                    $model->eliminarImagen($anterior['imgNoticia']);
                }
            }

            $ok = $model->updateNoticias($id, $titulo,$autor,$epigrafe, $contenido, $imagenNombre);
            header('Content-Type: application/json; charset=utf-8');
            echo json_encode(['success' => $ok, 'message' => $ok ? 'Actualizado' : 'No se pudo actualizar'], JSON_UNESCAPED_UNICODE);
            break;

        case 'delete':
            $id = intval($_POST['idNoticia'] ?? $_GET['idNoticia'] ?? 0);
            $ok = $model->deleteNoticias($id);
            header('Content-Type: application/json; charset=utf-8');
        ...
        case 'image':
            // cargar la imagen
            $titulo = $_GET['name'] ?? '';
            $path = __DIR__ . '/../assets/img/' . basename($titulo);
            if (is_file($path)) {

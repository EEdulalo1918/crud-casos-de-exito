...
include('../modelos/modBolsaUniModel.php');
...
try {
    switch ($action) {
        case 'list':
            $rows = $model->getAllBolsaUni();
            header('Content-Type: application/json; charset=utf-8');
            echo json_encode(['success' => true, 'data' => $rows], JSON_UNESCAPED_UNICODE);
            break;

        case 'get':
            $id = intval($_GET['idPlataforma'] ?? 0);
            $row = $model->getBolsaUni($id);
            header('Content-Type: application/json; charset=utf-8');
            if ($row) {
        ...
        case 'update':
            ...
            if (!empty($_FILES['imagen']['tmp_name'])) {
                ...
                $imagenNombre = $model->guardarImagen($_FILES['imagen']);
                if ($anterior && !empty($anterior['imgPlataforma']) && $anterior['imgPlataforma'] !== $imagenNombre) {
                    $model->eliminarImagen($anterior['imgPlataforma']);
                }
            }

            $ok = $model->updateBolsaUni($id, $nombre, $url, $imagenNombre);
            header('Content-Type: application/json; charset=utf-8');
            echo json_encode(['success' => $ok, 'message' => $ok ? 'Actualizado' : 'No se pudo actualizar'], JSON_UNESCAPED_UNICODE);
            break;

        case 'delete':
            $id = intval($_POST['idPlataforma'] ?? $_GET['idPlataforma'] ?? 0);
            $ok = $model->deleteBolsaUni($id);
            header('Content-Type: application/json; charset=utf-8');
